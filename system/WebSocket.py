...
def add_face():
    if request.method == 'POST':
        ...
        if new_name is not "trust":
            wriitenToDir = Home_Surveillance.add_face(new_name,img)
        else:
            wriitenToDir = Home_Surveillance.add_face(predicted_name,img)
            ...
            for i, camera in enumerate(Home_Surveillance.cameras):
                with Home_Surveillance.cameras[i].people_dict_lock:
                    for key, obj in camera.people.iteritems():  
                        persondict = {'identity': key , 'confidence': obj.confidence, 'camera': i, 'timeD':obj.time, 'prediction': obj.get_prediction()}
                        print persondict
